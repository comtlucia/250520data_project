...
if elderly_ratio >= 25:
    summary += (
        "이 지역은 고령 인구가 상대적으로 많습니다. "
        "노인 복지관, 실버문화센터, 한방 병원, 무장애 보행환경 등 고령자 친화 인프라 확보가 필요합니다. "
        "노인 단골 상점이나 전통시장 활성화도 중요합니다."
    )

elif youth_ratio >= 30:
    summary += (
        "청년층이 중심을 이루는 지역으로, 창업 지원센터, 공유 오피스, 임대 주택, 야간 카페, 문화예술 공간 등의 "
        "유연한 생활 기반 조성이 유리합니다. 청년 커뮤니티와 스타트업 생태계 구축도 고려해볼 수 있습니다."
    )

elif under20_ratio >= 25:
    summary += (
        "어린이와 청소년 비중이 높은 지역입니다. "
        "놀이공간, 방과후 돌봄센터, 학원가, 청소년 문화의 집과 같은 교육 및 여가 인프라가 필요합니다. "
        "안전한 등하굣길과 공공 CCTV 강화도 고려 대상입니다."
    )

elif middle_ratio >= 35:
    summary += (
        "중장년층 비중이 높은 지역입니다. 평생학습관, 직장인 건강센터, 중년 재취업센터, 여가복합시설 등 "
        "자기계발과 건강관리 중심의 공간이 유효합니다. 편의점, 약국, 대형마트 중심의 생활상권도 강세입니다."
    )

else:
    summary += (
        "전 세대가 비교적 고르게 분포된 균형형 지역입니다. "
        "복합문화공간, 가족 단위 여가시설, 공공 도서관, 커뮤니티센터 등 다양한 세대가 공존할 수 있는 "
        "복합적 공간 구성이 바람직합니다."
    )
st.markdown(summary)
#st.info(summary)
